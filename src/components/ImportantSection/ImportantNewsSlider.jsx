...
const ImportantNewsSlider = ({ data }) => {
  ...
  return (
    data.length > 0 && (
      ...
      >
        <div className='mb-5 h-[465px] w-[855px] bg-clip-content pr-10'>
          <img
            ...
            className='h-[465px] w-full object-cover animate-fadeIn'
            alt='slide'
          />
        </div>
        <div
        ...
        <div className='absolute top-0 z-20 flex w-[855px] translate-y-60 flex-col justify-start gap-5 bg-clip-content pr-20 pb-1 pl-5 text-base-100'>
          <h3 className='flex justify-center items-center w-24 p-1 border-2 border-color-150 leading-none'>
            ВАЖНОЕ
          </h3>
      ...
      <button className="block absolute top-[50%] z-50 duration-500 -translate-x-80 right-[14%] text-2xl rounded-full p-2 bg-black/20 text-white cursor-pointer">
                        <HiChevronRight
                            onClick={goToNextSlide}
                        />
      </button>
      <button className="block absolute top-[50%] z-50 duration-500  left-[38px] text-2xl rounded-full p-2 bg-black/20 text-white cursor-pointer">
              <HiChevronLeft
